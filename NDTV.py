...
def store_news_in_csv():
    threading.Timer(600.0, store_news_in_csv).start()
    L = []
    for category in main_categories:
        df = fetch_main_news_data(
            category = category, news_page_url = main_categories[category]
        )
        L.append(df)
...
def read_dataframe(
    ...
    return {"status": "successfully fetched", "news": output_category_list}
...
class LatestNews(Resource):
    def get(self):
        ...
        if user_requested_field is not None and user_requested_category is not None:
            ...
            category_list = re.findall(r"([^(,)]+)(?!.*\()", user_requested_category)
            news_list = read_dataframe(requested_categories=category_list)
